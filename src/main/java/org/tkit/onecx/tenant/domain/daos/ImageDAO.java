...
import org.tkit.onecx.tenant.domain.models.Image;
import org.tkit.onecx.tenant.domain.models.Image_;
...
public class ImageDAO extends AbstractDAO<Image> {
    ...
    public Image findByRefIdAndRefType(String refId, String refType) {
        ...
        try {
            ...
            throw new DAOException(ErrorKeys.FIND_ENTITY_BY_REF_ID_REF_TYPE_FAILED, ex);
        }
    }
    ...
    public void deleteQueryByRefIdAndRefType(String refId, RefTypeDTO refType) throws DAOException {
        try {
            ...
            var root = cq.from(Image.class);
            var cb = this.getEntityManager().getCriteriaBuilder();
            cq.where(cb.and(cb.equal(root.get(Image_.REF_ID), refId)), cb.equal(root.get(Image_.REF_TYPE), refType.toString()));
